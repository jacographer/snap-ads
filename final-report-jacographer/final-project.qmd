...
The goals of this analysis, building on the submission to my Mapping Block Report, are to hone in on particular patterns of targeting across both **time** and **target audience**. This is in contrast to the previous report, which mapped the total advertisements targeting a state without regard to time or audience.
...
max_list |>
  ...
  facet_wrap(~STATE_INC)+
  theme_bw()+
  theme(rect = element_rect(fill = "antiquewhite"), legend.position="bottom", text = element_text(size = 20))
...
tm_sum |>
  ...
  theme_bw()+
  theme(rect = element_rect(fill = "antiquewhite"), text = element_text(size = 20))
```
